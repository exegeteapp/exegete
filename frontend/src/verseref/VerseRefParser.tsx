...
const makeBookLookup = (module: ModuleInfo) => {
    ...
    book_abbrevs.forEach((abbrevs_set: Set<string>, book: string) => {
        ...
        const book_re = new RegExp(`(${abbrevs.map(escapeRegExp).join('|')})`);
...
export type AbsoluteScriptureRef = 
    ...
    | { "type": "ref", "abs": true, "to": "book_chapter", "book": string, "chapter": RefNumber }
    | { "type": "ref", "abs": true, "to": "book_chapter_verse", "book": string, "chapter": RefNumber, "verse": RefNumber }

export type ContextualScriptureRef = 
    | { "type": "ref", "abs": false, "to": "chapter_verse", "chapter": RefNumber, "verse": RefNumber }
...
export const makeLanguage = (module: ModuleInfo) => {
    ...
    let lang = P.createLanguage<{
        ...
        b_c_v: r => P.seqMap(
            ...
            r.ref_number,
            (b, c, _, v) => ({ type: "ref", "abs": true, "to": "book_chapter_verse", book: b, chapter: c, verse: v })),
        b_c: r => P.seqMap(
            r.book.skip(P.whitespace),
            r.ref_number,
            (b, c) => ({ "type": "ref", "abs": true, "to": "book_chapter", book: b, chapter: c })),
        c_v: r => P.seqMap(
            r.ref_number.skip(P.optWhitespace),
            P.string(':').skip(P.optWhitespace),
            r.ref_number,
            (c, _, v) => ({ type: "ref", "abs": false, "to": "chapter_verse", chapter: c, verse: v })),
        b: r => r.book.map(b => ({ type: "ref", "abs": true, "to": "book", book: b })),
        c_or_v: r => r.ref_number.map(v => ({ "type": "ref", "abs": false, "to": "chapter_or_verse", value: v })),

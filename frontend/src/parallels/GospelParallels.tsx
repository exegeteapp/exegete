...
type Database = [GospelParallelText, ScriptureBookChapters][];
...
export const ParallelSearch = (
    ...
    const matches: GospelParallelText[] = [];
    const s_sbcs = parse();
    const s_lower = s.toLowerCase();

    db.forEach(([parallel, entries]) => {
        if (parallel.title.toLowerCase().includes(s_lower)) {
            matches.push(parallel);
            return;
        }
        if (s_sbcs) {
